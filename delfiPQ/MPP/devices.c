...
void device_init() {
  ...
  for(i = 0; i < MAX_INA_DEVS; i++) {
    ina_dev[i].id = i + MPP_BOARD_MON_DEV_ID;
    ina_dev[i].voltage = 0;
    ina_dev[i].current = 0;
    ina_dev[i].power = 0;
...
void update_device(dev_id id) {
  ...
  if(id == MPP_BOARD_MON_DEV_ID ||
    ...
    uint8_t pos_index = id - MPP_BOARD_MON_DEV_ID;
    ...
    uint8_t pos_index = id - MPP_TEMP_DEV_ID;
